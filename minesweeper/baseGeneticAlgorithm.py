...
class basicGA(baseGeneticAlgorithm.baseGeneticAlgorithm):
    ...
    def fitnessFunction(self, solution):
        ...
        for node in solution:
            if node == self.board[y_cord][x_cord] and node == 0:   # Node in chromosome matches the board
                score = score + 1
            elif node == self.board[y_cord][x_cord] and node == 1:   # Node in chromosome matches the board
                score = score + 2
            elif node == 0 and self.board[y_cord][x_cord] == 1:    # Mistakes empty tile for bomb tile - loss
                return score + self.checkBombLocation(solution)
    ...
    def checkBombLocation(self, solution):
        score, x_cord, y_cord = 0, 0, 0
        for node in solution:
            if node == 1:
